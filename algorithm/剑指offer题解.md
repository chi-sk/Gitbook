...
public:
    TreeNode* reConstructBinaryTree(vector<int> pre,vector<int> vin) {
        ...
        TreeNode* pRoot = new TreeNode(pre[0].val);
        int nIndex = 0;
        for(int i = 0; i < nSize; ++i)
        {
            if(vin[i].val == pRoot->val)
            {
                nIndex = i;
                break;
            }
        ...
        {
            inLeft.push_back(vin[i].val);
            preLeft.push_back(pre[i + 1].val);
        }
        for(int j = nIndex; j < nSize; ++j)
        {
            inRight.push_back(vin[nIndex + 1].val);
            preRight.push_back(pre[nIndex + 1].val);
        }
        pRoot->left = reConstructBinaryTree(preLeft,inLeft);
        pRoot->right = reConstructBinaryTree(preRight,inRight);
        return pRoot;
    }
};
```
